...
impl DigestAlgorithm {
    ...
    pub fn hash_hex(&self, data: &mut impl Read) -> Result<HexDigest> {
        ...
        io::copy(&mut hasher, &mut io::sink()).context(IoGeneralSnafu {})?;
...
mod tests {
    ...
    #[test]
    fn calculate_digest_while_writing() {
        let input = "testing\n".to_string();
        let output: Vec<u8> = Vec::new();

        let mut writer = DigestAlgorithm::Sha512.writer(output);

        io::copy(&mut input.as_bytes(), &mut writer).unwrap();
    ...
    #[test]
    fn calculate_multiple_digests_while_writing() {
        let input = "testing\n".to_string();
        let output: Vec<u8> = Vec::new();
